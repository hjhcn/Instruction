 ...
 * ================================================ UC Client 通用调用该类来实现与UC Server之间的通信。 该类实现与UC
 * Server通信的所有接口函数
 * 
...
public class UCClient extends PHPFunctions {
	...
	public static String UC_IP = "120.195.11.149";
	public static String UC_API = "http://120.195.11.149/uc_server/index.php";
	public static String UC_CONNECT = "";
	public static String UC_KEY = "&*&*&*&*&*(*$%($*(%*$(";
	public static String UC_APPID = "8";
	/**
	 * 来福币code
	 */
	public static String UC_LFB_CODE = "80001";
	public static String UC_CLIENT_VERSION = "1.0";
	public static String UC_CLIENT_RELEASE = "20120520";
	...
	public String uc_api_post(String $module, String $action, Map<String, Object> $arg) {
		...
		for (String $k : $arg.keySet()) {
			...
			if ($v.getClass().isAssignableFrom(Map.class)) {
				...
				$s.append($sep).append($k).append("=")
						.append(urlencode(uc_stripslashes(String.valueOf($v))));
			}
			$sep = "&";
		}
	...
	protected String uc_api_requestdata(String $module, String $action, String $arg, String $extra) {
		...
		String $post = "m=" + $module + "&a=" + $action + "&inajax=2&release=" + UC_CLIENT_RELEASE
	...
	public String uc_api_input(String $data) {
		...
		// "ENCODE", UC_KEY));
		String $s = urlencode(uc_authcode($data + "&agent=" + md5("") + "&time=" + time(),
				"ENCODE", UC_KEY));
		return $s;
	}
	 ...
	 * @return mix
	 */
	public String uc_api_mysql(String $model, String $action, Map<String, Object> $args) {
		// global $uc_controls;
	 ...
	 *          $b(abc)
	 * 
	 *          $a = authcode('abc', 'ENCODE', 'key', 3600); $b = authcode('abc', 'DECODE', 'key');
	 *          // 在一个小时内，$b(abc)，否则 $b 为空
	 */
	...
	public String uc_authcode(String $string, String $operation, String $key, int $expiry) {
		...
		String $keyc = $ckey_length > 0 ? ($operation.equals("DECODE") ? substr($string, 0,
				$ckey_length) : substr(md5(microtime()), -$ckey_length)) : "";

		String $cryptkey = $keya + md5($keya + $keyc);
		int $key_length = $cryptkey.length();

		$string = $operation.equals("DECODE") ? base64_decode(substr($string, $ckey_length))
				: sprintf("%010d", $expiry > 0 ? $expiry + time() : 0)
		...
		if ($operation.equals("DECODE")) {
			...
			if ((Integer.parseInt(substr($result.toString(), 0, 10)) == 0 || Long.parseLong(substr(
					$result.toString(), 0, 10)) - time() > 0)) {
				return substr($result.toString(), 26);
			} else {
				return "";
			}
	 ...
	 * @return 取到的字符串
	 */
	protected String uc_fopen2(String $url, int $limit, String $post, String $cookie,
			boolean $bysocket, String $ip, int $timeout, boolean $block) {
	...
	protected String uc_fopen(String $url, int $limit, String $post, String $cookie,
			boolean $bysocket, String $ip, int $timeout, boolean $block) {
		String $return = "";
	 ...
	 * @param string
	 *            $answer 安全提问答案
	 * @return int -1 : 用户名不合法 -2 : 包含不允许注册的词语 -3 : 用户名已经存在 -4 : email 格式有误 -5 : email 不允许注册 -6 : 该
	 *         email 已经被注册 >1 : 表示成功，数值为 UID
	 */
	 ...
	 * @param string
	 *            $answer 安全提问答案
	 * @return int 1 : 修改成功 0 : 没有任何修改 -1 : 旧密码不正确 -4 : email 格式有误 -5 : email 不允许注册 -6 : 该 email
	 *         已经被注册 -7 : 没有做任何修改 -8 : 受保护的用户，没有权限修改
	 */
	public String uc_user_edit(String $username, String $oldpw, String $newpw, String $email,
			int $ignoreoldpw, String $questionid, String $answer) {
		Map<String, Object> args = new HashMap<String, Object>();
		args.put("username", $username);
	 ...
	 *            手机号码
	 * @return 返回credit
	 * @return -1:用户不存在;-4:不是苏州生活网用户或者来福币余额不足;其他非负值:用户来福币
	 * @throws Exception
	 */
	public String uc_credit_querylfb(String smsphone) throws Exception {
		Map<String, Object> args = new HashMap<String, Object>();
		args.put("tel", smsphone);
		return uc_api_post("credit", "querylfb", args);
	}
	 ...
	 * @throws Exception
	 */
	public String uc_credit_operatelfb(String smsphone, int uid, int num, String oreason,
			String orderNumber) throws Exception {
		String phoneNumber = smsphone;
		if (!Utils.isPhoneNumber(phoneNumber)) {
			LinkedList<String> rs = XMLHelper.uc_unserialize(this.uc_get_user(uid + "", 1));
			if (rs.size() >= 4) {
				phoneNumber = rs.get(3);
			}
		}
		System.out.println("来福币请求，手机号码：" + phoneNumber + "订单号：" + orderNumber + ",原因：" + oreason);
		Map<String, Object> args = new HashMap<String, Object>();
		args.put("tel", phoneNumber);
		args.put("num", num);
		args.put("code", UC_LFB_CODE);
		args.put("oreason", oreason);
		args.put("no", orderNumber);
		String action = "addlfb";
		if (num < 0) {
			action = "reducelfb";
			num = -num;
		}
		String result = uc_api_post("credit", action, args);
	 ...
	 * 从code密文字符串返回Map格式数据
	 */
	public Map<String, String> decodeReturnMap(String code) {
		Map<String, String> codeMap = new HashMap<String, String>();
		String decode = this.uc_authcode(code, "DECODE");
		// System.out.println(decode);
		if (decode != null) {
			String[] paras = decode.split("&");
			for (String para : paras) {
				int equalIndex = para.indexOf("=");
				if (equalIndex > 0) {
					String name = para.substring(0, equalIndex);
					String value = para.substring(equalIndex + 1, para.length());
