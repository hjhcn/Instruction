...
jQuery(function($) {
	...
	$("#tempfile-copy").click(
			function() {
				...
				if (tempfile_copy) {
					...
					$.getJSON("/admin/json/ins_uploadTemp_fileList?time=" + new Date().getTime()
							/ 1000, function(data) {
	...
	$("#select-btn").click(
			function() {
				...
				if (filename != "") {
					...
					$.getJSON("/admin/json/ins_selectTempFile_fileFolderAndName?time="
	...
	$('#imgBtn')
			.uploadify(
					{
						'uploader' : '/admin/resources/swf/uploadify.swf',
						'script' : '/admin/json/upload_uploadFile_uploadFile;jsessionid='
								+ JSESSIONID, // 指定服务端处理类的入口
						'folder' : 'avatar',
						'cancelImg' : '/admin/resources/images/cancel.png',
	...
	$('#fileBtn').uploadify({
		'uploader' : '/admin/resources/swf/uploadify.swf',
		'script' : '/admin/json/upload_uploadFile_uploadFile;jsessionid=' + JSESSIONID, // 指定服务端处理类的入口
		'folder' : 'avatar',
		'cancelImg' : '/admin/resources/images/cancel.png',
		'fileDataName' : 'file', // 和input的name属性值保持一致就好，Struts2就能处理了
		'queueID' : 'fileQueue2',
	...
	$("#sub").click(
			function() {
				...
				if (verify) {
					...
					$.getJSON(
							"/admin/json/ins_upload_feedback?time=" + new Date().getTime() / 1000,
							param, function(data) {
								overallFeedbackHandle(data.feedback);
								if (data.feedback == 100)
									alert("上传成功");
	...
	$("#edit-sub").click(
			function() {
				...
				if (verify) {
					...
					$.post("/admin/json/ins_edit_feedback?time=" + new Date().getTime() / 1000,
							param, function(data) {
								overallFeedbackHandle(data.feedback);
								if (data.feedback == 100)
									alert("更新成功");
									...
									if (data.feedback == -602) {
										...
										if (result) {
											...
											if (result) {
												$.getJSON("/admin/json/ins_delete_?time="
	...
	$("#mobile").autocomplete(
			{
				...
				source : function(request, response) {
					$.ajax({
						url : "/admin/json/upload_list_uploadFiles",
						dataType : "json",
						data : {
							"search" : request.term,
							"ext" : "SWF",
	...
	$("#reCoverter").click(function() {
		var id = $("#act-id").val();
		$.getJSON("/admin/json/ins_reCoverter_?time=" + new Date().getTime() / 1000, {
			"id" : id
		}, function(data) {
			overallFeedbackHandle(data.feedback);
			if (data.feedback == 100)
	...
	$(".edit-link,.detail-link").click(
			function() {
				...
				$.getJSON("/admin/json/ins_ins_ins?time=" + new Date().getTime() / 1000, {
					id : iid
				}, function(data) {
					overallFeedbackHandle(data.feedback);
					var ins = data.ins;
	...
	$(".verify-link").click(function() {
		...
		$.getJSON("/admin/json/ins_" + action + "_?time=" + new Date().getTime() / 1000, {
			ids : iid
		}, function(data) {
			overallFeedbackHandle(data.feedback);
			if (data.feedback > 0) {
	...
	$(".delete-link").click(function() {
		...
		if (result) {
			$.getJSON("/admin/json/ins_delete_?time=" + new Date().getTime() / 1000, {
				ids : iid
			}, function(data) {
				overallFeedbackHandle(data.feedback);
				if (data.feedback == 1) {
	...
	$("#ins-set-apply").click(
			function() {
				...
				if (ids == "")
					...
					if (result) {
						$.getJSON("/admin/json/ins_" + operate + "_?time=" + new Date().getTime()
								/ 1000, {
							"ids" : ids
						}, function(data) {
							overallFeedbackHandle(data.feedback);
	...
	$("#model").blur(function() {
		$.getJSON("/admin/json/ins_judageModel_feedback?time=" + new Date().getTime() / 1000, {
	...
	function setBrandByCid(Cid, callback) {
		$("#bid").html("");
		$.getJSON("/admin/json/category_brandsByCid_brands?time=" + new Date().getTime() / 1000, {
			cid : Cid
		}, function(data) {
			overallFeedbackHandle(data.feedback);
	...
	$(".cat-edit-link,.cat-detail-link").click(
			function() {
				...
				$("#act-id").val(cid);
				$.getJSON("/admin/json/category_category_category?time=" + new Date().getTime() / 1000, {
					"cid" : cid
				}, function(data) {
					overallFeedbackHandle(data.feedback);
	...
	$(".cat-delete-link").click(function() {
		...
		if (result) {
			$.getJSON("/admin/json/category_delete_?time=" + new Date().getTime() / 1000, {
				"cid" : cid
			}, function(data) {
				overallFeedbackHandle(data.feedback);
				if (data.feedback == 100) {
	...
	$("#cat-edit-sub").click(
			function() {
				...
				if (verify) {
					...
					$.getJSON("/admin/json/category_edit_?time=" + new Date().getTime() / 1000,
							param, function(data) {
								overallFeedbackHandle(data.feedback);
								if (data.feedback == 100)
									alert("更新成功");
	...
	$("#cat-add-sub").click(
			function() {
				...
				if (verify) {
					...
					$.getJSON("/admin/json/category_add_?time=" + new Date().getTime() / 1000,
							param, function(data) {
								overallFeedbackHandle(data.feedback);
								if (data.feedback == 100)
									alert("添加成功");
	...
	$(".brand-edit-link,.brand-detail-link").click(function() {
		...
		$("#act-id").val(bid);
		$.getJSON("/admin/json/brand_brand_brand?time=" + new Date().getTime() / 1000, {
			"bid" : bid
		}, function(data) {
			overallFeedbackHandle(data.feedback);
	...
	$(".brand-delete-link").click(function() {
		...
		if (result) {
			$.getJSON("/admin/json/brand_delete_?time=" + new Date().getTime() / 1000, {
				"bid" : bid
			}, function(data) {
				overallFeedbackHandle(data.feedback);
				if (data.feedback == 100) {
	...
	$("#brand-edit-sub").click(
			function() {
				...
				if (verify) {
					...
					$.getJSON("/admin/json/brand_edit_?time=" + new Date().getTime() / 1000, param,
							function(data) {
								overallFeedbackHandle(data.feedback);
								if (data.feedback == 100)
									alert("更新成功");
	...
	$("#brand-add-sub").click(
			function() {
				...
				if (verify) {
					...
					$.getJSON("/admin/json/brand_add_?time=" + new Date().getTime() / 1000, param,
							function(data) {
								overallFeedbackHandle(data.feedback);
								if (data.feedback == 100)
									alert("添加成功");
	...
	$("#newPswd-sub").click(
			function() {
				...
				if (verify) {
					...
					$.getJSON("/admin/json/admin_password_feedback?time=" + new Date().getTime()
	...
	$(".cr-edit-link").click(function() {
		...
		$("#crid").val(crid);
		$.getJSON("/admin/json/credit_creditRule_creditRule?time=" + new Date().getTime() / 1000, {
			"crid" : crid
		}, function(data) {
			overallFeedbackHandle(data.feedback);
	...
	$("#cr-edit-sub").click(
			function() {
				...
				if (verify) {
					...
					$.getJSON("/admin/json/credit_editRule_feedback?time=" + new Date().getTime()
							/ 1000, param, function(data) {
						overallFeedbackHandle(data.feedback);
						if (data.feedback == 100)
							alert("修改成功!");
	...
	$(".cl-recreidt-link").click(function() {
		...
		$.getJSON("/admin/json/credit_reCredit_feedback?time=" + new Date().getTime() / 1000, {
			"ids" : clid
		}, function(data) {
			overallFeedbackHandle(data.feedback);
			if (data.feedback > 0) {
	...
	$("#credit-manual-sub").click(function() {
		...
		var credit = $("#credit").val();
		var description = $("#description").val();
		$.getJSON("/admin/json/credit_manual_feedback?time=" + new Date().getTime() / 1000, {
			"uid" : uid,
			"credit" : credit,
			"description" : description
		}, function(data) {
	...
	$(".comment-credit-link").click(function() {
		...
		$.getJSON("/admin/json/comment_credit_feedback?time=" + new Date().getTime() / 1000, {
			"ids" : cid,
	...
	$("#bbsthread-set-apply").click(
			function() {
				...
				if (inputArrayStr != "") {
					$.getJSON("/admin/json/bbsThread_edit_feedback?time=" + new Date().getTime()
							/ 1000, {
	...
	$("#bbsThread-add-sub").click(
			function() {
				...
				if (verify) {
					...
					$.getJSON("/admin/json/bbsThread_add_?time=" + new Date().getTime() / 1000,
							param, function(data) {
								overallFeedbackHandle(data.feedback);
								if (data.feedback == 100)
									alert("添加成功");
	...
	$(".bbs-delete-link").click(function() {
		...
		if (result) {
			$.getJSON("/admin/json/bbsThread_delete_?time=" + new Date().getTime() / 1000, {
				"id" : tid
			}, function(data) {
				overallFeedbackHandle(data.feedback);
				if (data.feedback > 0) {
	...
	$("#news-set-apply").click(function() {
		...
		if (ids != "") {
			$.getJSON("/admin/json/news_operate_feedback?time=" + new Date().getTime() / 1000, {
				"ids" : ids,
				"operate" : operate
			}, function(data) {
				overallFeedbackHandle(data.feedback);
	...
	$("#news-add-sub").click(
			function() {
				...
				if (verify) {
					...
					$.post("/admin/json/news_add_feedback?time=" + new Date().getTime() / 1000,
	...
	$("#news-edit-sub").click(
			function() {
				...
				if (verify) {
					...
					$.post("/admin/json/news_edit_feedback?time=" + new Date().getTime() / 1000,
							param, function(data, state) {
								overallFeedbackHandle(data.feedback);
								if (data.feedback == 100)
									alert("提交成功");
	...
	$(".news-edit-link").click(function() {
		...
		$("#act-id").val(id);
		$.getJSON("/admin/json/news_news_?time=" + new Date().getTime() / 1000, {
			"id" : id
		}, function(data) {
			overallFeedbackHandle(data.feedback);
			var news = data.news;
...
function listUploadPic() {
	$.getJSON("/admin/json/upload_list_uploadFiles?time=" + new Date().getTime() / 1000, {
		'ext' : 'PIC',
